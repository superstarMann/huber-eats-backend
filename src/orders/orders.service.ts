...
export class OrderService{
    ...
    async createOrder(
        customer: User, 
        {storeId, items}: CreateOrderInput): Promise<CreateOrderOutput>{
        try{
            ...
            await this.pubSub.publish(NEW_PENDING_ORDER, { pendingOrders: {order, ownerId: store.ownerId} });
            return{
                ok: true,
    ...
    async getOrders(user: User, {status}: GetOrdersInput): Promise<GetOrdersOutput>{
      ...
      async getOrder(user: User, {id: orderId}: GetOrderInput): Promise<GetOrderOutput>{
          try{
              const order = await this.orders.findOne(orderId, {relations: ['store']});
              if(!order){
                  return{
                      ok: false,
